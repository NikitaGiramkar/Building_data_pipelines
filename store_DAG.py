...
with DAG('store_dag',default_args=default_args,schedule_interval='@daily', template_searchpath=['/home/nikita/airflow/sql_files'], catchup=True) as dag:
    download = PythonOperator(task_id='data_download', python_callable=data_downloader)

    check_file =BashOperator(task_id='check_file_exists', bash_command='shasum /home/nikita/airflow/input_raw/raw_store_transactions.csv', retries=2, retry_delay=timedelta(seconds=15))
    ...
    #t7=BashOperator(task_id='Open_App', bash_command='streamlit run /home/nikita/airflow/dags/MLapp-Final.py & disown', retries=2, retry_delay=timedelta(seconds=15))
